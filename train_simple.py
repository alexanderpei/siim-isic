...
# Now that we've created our image data generators, we need to specify where to load the images from.
...
# Need to specify how many batches we want to use during training. The method below will make sure that each image
# gets passed through every epoch. If your computer is slow, you can change float(len(trainIm.filenames)) to
# something smaller than the total number of images (roughly 40,000, try changing to 2000 for speed).
...
# Creating a pandas data frame with the names of all of the images
